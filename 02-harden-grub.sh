...
GRUB_SECURITY_FILE="/etc/grub.d/01_security"
cat > "${GRUB_SECURITY_FILE}" <<EOF
# This file is automatically generated by the hardening scripts.
# It sets a password to protect the GRUB boot menu.

set superusers="grub_admin"
password_pbkdf2 grub_admin ${GRUB_PASSWORD_HASH}

# --- Optional: Restrict boot entries ---
# To ensure that no boot entries can be started without a password,
# you can add '--unrestricted' to only one specific menuentry in other
# /etc/grub.d files. For maximum security, we leave this out,
# requiring the password for any boot action.
EOF
